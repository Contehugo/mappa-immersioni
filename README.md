Papa.parse(csvUrl, {
            ...
            complete: function(results) {
                results.data.forEach(function(row) {
                    ...
                    if (lat && lng) {
                        // Creiamo un'icona personalizzata solo se c'è un link nella colonna IconaURL
                        var marker;
                        if (row.IconaURL && row.IconaURL.trim() !== "") {
                            var iconaPersonalizzata = L.icon({
                                iconUrl: row.IconaURL.trim(),
                                iconSize: [40, 40], // Dimensione in pixel (puoi cambiarla)
                                iconAnchor: [20, 20]
                            });
                            marker = L.marker([lat, lng], {icon: iconaPersonalizzata, title: row.Nome});
                        } else {
                            // fallback al pallino se manca l'icona
